...
class CertFactory:
    ...
    def getPrivateKeyForAddr(addr):
        ...
        if addr == "20174.1.636.300":
            with open("/home/zifan/Cert/client-prikey")as fp:
                private_key_user = fp.read()
            return private_key_user
        if addr == "20174.1.636.200":
            with open("/home/zifan/Cert/server-prikey")as fp:
                private_key_user = fp.read()
            return private_key_user
        return None
    ...
    def getCertsForAddr(addr):
        ...
        if addr == "20174.1.636.300":
            with open("/home/zifan/Cert/signed-client.cert", 'rb')as fo:
                chain.append(fo.read())
            with open("/home/zifan/Cert/signed.cert", 'rb')as fi:
                chain.append(fi.read())
            return chain
        if addr == "20174.1.636.200":
            with open("/home/zifan/Cert/signed-server.cert", 'rb')as fo:
                chain.append(fo.read())
            with open("/home/zifan/Cert/signed.cert", 'rb')as fi:
                chain.append(fi.read())
            return chain
    ...
    def getRootCert():
        ...
        # system
        with open("/home/zifan/Cert/root.crt","rb")as fp:
            rootcert = fp.read()
        return rootcert
