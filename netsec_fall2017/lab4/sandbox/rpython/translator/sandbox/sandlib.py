...
def write_message(g, msg, resulttype=None):
    if resulttype is None:
        ...
        st = tuple(msg)
        st = st[:-3] + (float(st[-3]), float(st[-2]), float(st[-1]))
        marshal.dump(st, g, 0)
...
def write_exception(g, exception, tb=None):
    for i, excclass in EXCEPTION_TABLE:
        ...
        raise exception#exception.__class__, exception, tb
    ...
    def interact(self, stdin=None, stdout=None, stderr=None):
        ...
        if returncode != 0:
            if os.name == 'posix' and returncode < 0:
                print("[Subprocess killed by {}]".format(signal_name(-returncode)), file=self._error)
                # >> self._error, "[Subprocess killed by %s]" % (
                #    signal_name(-returncode),)
            else:
                print("[Subprocess exit code: {}]".format(returncode), file=self._error)
...
class VirtualizedSandboxedProc(SimpleIOSandboxedProc):
    ...
    def do_ll_sys__ll_get_pypy_path(self):
        return self.pypy_stdlib_path
    ...
    def do_ll_os__ll_os_fstat(self, fd):
        if fd in (0, 1, 2):
            if fd == 0: realFd = self._input.fileno()
            elif fd == 1: realFd = self._output.fileno()
            elif fd == 2: realFd = self._error.fileno()
            return os.fstat(realFd)
    ...
    def do_ll_os__ll_os_listdir(self, vpathname):
        node = self.get_node(vpathname)
        return list(node.keys()) # force it to list because might be dictkeys
